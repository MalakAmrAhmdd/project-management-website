...
export const api = {
  ...
  updateProject: (id: number, data: Partial<Project>) => request<Project>(`/projects/${id}`, { method: "PATCH", body: JSON.stringify(data) }),
  deleteProject: (id: number) => request<void>(`/projects/${id}`, { method: "DELETE" }),

  // Phases
  listPhases: (projectId: number) => request<Phase[]>(`/phases/?project_id=${projectId}`),
  createPhase: (data: Partial<Phase> & { project_id: number }) => request<Phase>("/phases/", { method: "POST", body: JSON.stringify(data) }),
  updatePhase: (id: number, data: Partial<Phase>) => request<Phase>(`/phases/${id}`, { method: "PATCH", body: JSON.stringify(data) }),
  ...
  // Epics
  listEpics: (milestoneId: number) => request<Epic[]>(`/epics/?milestone_id=${milestoneId}`),
  createEpic: (data: Partial<Epic> & { milestone_id: number }) => request<Epic>("/epics/", { method: "POST", body: JSON.stringify(data) }),
  updateEpic: (id: number, data: Partial<Epic>) => request<Epic>(`/epics/${id}`, { method: "PATCH", body: JSON.stringify(data) }),
  deleteEpic: (id: number) => request<void>(`/epics/${id}`, { method: "DELETE" }),

  // Stories
  listStories: (epicId: number) => request<Story[]>(`/stories/?epic_id=${epicId}`),
  createStory: (data: Partial<Story> & { epic_id: number }) => request<Story>("/stories/", { method: "POST", body: JSON.stringify(data) }),
  updateStory: (id: number, data: Partial<Story>) => request<Story>(`/stories/${id}`, { method: "PATCH", body: JSON.stringify(data) }),
  ...
  createAllocation: (data: Partial<Allocation>) => request<Allocation>("/allocations/", { method: "POST", body: JSON.stringify(data) }),
  updateAllocation: (id: number, data: Partial<Allocation>) => request<Allocation>(`/allocations/${id}`, { method: "PATCH", body: JSON.stringify(data) }),
  deleteAllocation: (id: number) => request<void>(`/allocations/${id}`, { method: "DELETE" }),

  // Changelog
  listChangelog: (params?: { entity_type?: string; entity_id?: number; limit?: number }) => {
    const qs = new URLSearchParams();
  ...
  projectTimeline: () => request<TimelineProject[]>("/dashboard/project-timeline"),
};
